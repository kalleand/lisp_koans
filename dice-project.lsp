...
;;  YOU WRITE THIS PART:
(defclass dice-set ()
  ((value :reader get-values :writer roll :initform '())))

(defmethod get-values ((object dice-set))
  (slot-value object 'value))

(defmethod roll (how-many (object dice-set))
  ;; Loop how-many times and collect the values and save them into 
  ;; the local variable roll. This local variable is then set as the
  ;; value for the object.
  (let ((rolled (loop repeat how-many collect (+ 1 (random 6)))))
    (setf (slot-value object 'value) rolled)))
