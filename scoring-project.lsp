...
(defun count-numbers (num dice)
  (cond
    ((null dice) 0)
    ((= num (car dice)) (+ 1 (count-numbers num (cdr dice))))
    (t (count-numbers num (cdr dice)))))

(defun remove-numbers (num left dice)
  (cond
    ((null dice) '())
    ((= left 0) dice)
    ((= num (car dice)) (remove-numbers num (- left 1) (cdr dice)))
    (t (cons (car dice) (remove-numbers num left (cdr dice))))))

(defun score-ones (dice)
  (+ (* (count-numbers 1 dice) 100) (* (count-numbers 5 dice) 50)))

(defun score (dice)
  (cond
    ((null dice) 0)
    ((>= (count-numbers 1 dice) 3) (+ 1000 (score-ones (remove-numbers 1 3 dice))))
    ((>= (count-numbers 2 dice) 3) (+ 200 (score-ones (remove-numbers 2 3 dice))))
    ((>= (count-numbers 3 dice) 3) (+ 300 (score-ones (remove-numbers 3 3 dice))))
    ((>= (count-numbers 4 dice) 3) (+ 400 (score-ones (remove-numbers 4 3 dice))))
    ((>= (count-numbers 5 dice) 3) (+ 500 (score-ones (remove-numbers 5 3 dice))))
    ((>= (count-numbers 6 dice) 3) (+ 600 (score-ones (remove-numbers 6 3 dice))))
    (t (score-ones dice))))
